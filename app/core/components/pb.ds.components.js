(function () {
  ...
  angular.module('pb.ds.components').run(function ($templateCache) {
    var pbDsCheckBoxTemplate = '<span class="dscheckbox"><label class="control checkbox" ng-required="ctrl.required" ng-class="{\'disabled\':ctrl.disabled}"><input name="{{ctrl.name}}" ng-disabled="ctrl.disabled" type="checkbox" ng-model="ctrl.ngModel" ng-change="ctrl.ngChange({val:ctrl.ngModel})"><span class="control-indicator"></span> {{ctrl.label}} </label><span ng-transclude></span></span>';
